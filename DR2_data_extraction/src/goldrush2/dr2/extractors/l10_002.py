"""L10-002 extractor: CFTC total open interest."""
...
CACHE_PATH = PROJECT_ROOT / "data" / "cache" / "L10-002.json"
OUTPUT_PATH = PROJECT_ROOT / "data" / "current" / "L10-002.json"
...
def build_output(observations: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for horizon, (lookback, confidence) in HORIZONS.items():
        if latest is None or len(rows) <= lookback:
            ...
            continue
        old = rows[-lookback - 1]
        change = int(latest["open_interest"]) - int(old["open_interest"])
        output["horizons"][horizon] = {"signal": 1 if change > 0 else -1 if change < 0 else 0, "confidence": confidence, "evidence": {"current_open_interest": latest["open_interest"], f"{lookback}_weeks_ago_open_interest": old["open_interest"], f"{lookback}_weeks_ago_date": old["date"], "change": change}}
    return output
...
def run(*, output_path: Path = OUTPUT_PATH, cache_path: Path = CACHE_PATH) -> dict[str, Any]:
    try:
        ...
        raise ValueError(f"Cannot read L10-002 cache: {cache_path}") from exc
