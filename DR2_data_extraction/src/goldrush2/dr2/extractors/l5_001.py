...
from goldrush2.dr2.extractors._wgc_common import HORIZON_LOOKBACKS, build_schedule_bound_output, degraded, parse_official_changes_workbook
...
def build_output(observations: list[dict[str, Any]], *, cached: bool = False, as_of_date: str | None = None) -> dict[str, Any]:
    ...
    return build_schedule_bound_output(VARIABLE_ID, SOURCE_NAME, SOURCE_URL, observations, cached=cached, as_of_date=as_of_date, value_label="Official-sector net purchases", schedule_days=11, not_applicable_horizons={"1-5d"}, hold_latest_pending=False, rising_signal=1, falling_signal=-1)
...
def run(*, output_path: Path = OUTPUT_PATH, raw_dir: Path = RAW_DIR) -> dict[str, Any]:
    ...
    if workbook is None:
        ...
        try:
            output = build_schedule_bound_output(VARIABLE_ID, SOURCE_NAME, SOURCE_URL, parse_purchases_workbook(workbook), cached=wgc.LAST_FETCH_USED_CACHE, as_of_date=date.today().isoformat(), value_label="Official-sector net purchases", schedule_days=11, not_applicable_horizons={"1-5d"}, hold_latest_pending=False, rising_signal=1, falling_signal=-1, retrieved_at=datetime.now(timezone.utc).isoformat())
