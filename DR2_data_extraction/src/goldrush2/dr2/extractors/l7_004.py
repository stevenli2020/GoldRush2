"""DR2 extractor for L7-004: ICE BofA high-yield OAS."""
...
SOURCE_NAME = "FRED BAMLH0A0HYM2 - ICE BofA US High Yield Index Option-Adjusted Spread"
...
def _valid(current: dict[str, str | float], comparison: dict[str, str | float], *, cached: bool) -> dict[str, Any]:
    ...
    if change > 0:
        signal, summary = 1, f"High-yield credit spread widened by {change:.2f} percentage points, bullish for gold."
    elif change < 0:
        signal, summary = -1, f"High-yield credit spread narrowed by {abs(change):.2f} percentage points, bearish for gold."
    else:
        signal, summary = 0, "High-yield credit spread was unchanged, neutral for gold."
    if cached:
...
def build_output(observations: list[dict[str, str | float]], *, cached: bool = False, as_of_date: str | None = None) -> dict[str, Any]:
    """Build the four-horizon BAMLH0A0HYM2 result from valid daily observations."""
    ...
    for horizon, lookback in HORIZON_LOOKBACKS.items():
        if current is None or len(ordered) < lookback:
            ...
            summary = f"MISSING DATA — {lookback} valid daily observations are required; {len(ordered)} are available."
            if cached:
                summary += " SOURCE UNAVAILABLE — cached data used."
...
def run(*, raw_path: Path = RAW_PATH, output_path: Path = OUTPUT_PATH) -> dict[str, Any]:
    """Fetch BAMLH0A0HYM2, apply cache rules, and write current output."""
    try:
