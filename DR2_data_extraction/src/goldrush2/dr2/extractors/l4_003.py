"""DR2 extractor for L4-003: 5-year breakeven inflation."""
...
VARIABLE_ID = "L4-003"
SERIES_ID = "T5YIE"
SOURCE_NAME = "FRED T5YIE - 5-Year Breakeven Inflation Rate"
...
def _valid(current: dict[str, str | float], comparison: dict[str, str | float], *, cached: bool) -> dict[str, Any]:
    ...
    if change < 0:
        signal, summary = 1, f"5Y breakeven inflation fell by {abs(change):.2f} percentage points, bullish for gold."
    elif change > 0:
        signal, summary = -1, f"5Y breakeven inflation rose by {change:.2f} percentage points, bearish for gold."
    else:
        signal, summary = 0, "5Y breakeven inflation was unchanged, neutral for gold."
    if cached:
...
def build_output(observations: list[dict[str, str | float]], *, cached: bool = False, as_of_date: str | None = None) -> dict[str, Any]:
    """Build the four-horizon L4-003 result from valid FRED observations."""
...
def run(*, raw_path: Path = RAW_PATH, output_path: Path = OUTPUT_PATH) -> dict[str, Any]:
    """Fetch T5YIE, apply seven-day cache rules, and write current output."""
    try:
