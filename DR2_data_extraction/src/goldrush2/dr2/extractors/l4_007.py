"""DR2 extractor for L4-007: quarterly federal debt as a share of GDP."""
...
SOURCE_NAME = "FRED GFDEGDQ188S - Federal Debt/GDP Ratio"
...
def _valid(current: dict[str, str | float], comparison: dict[str, str | float], *, cached: bool) -> dict[str, Any]:
    ...
    if change < 0:
        signal, summary = 1, f"Debt/GDP ratio fell by {abs(change):.2f} percentage points, bullish for gold."
    elif change > 0:
        signal, summary = -1, f"Debt/GDP ratio rose by {change:.2f} percentage points, bearish for gold."
    else:
        signal, summary = 0, "Debt/GDP ratio was unchanged, neutral for gold."
    if cached:
...
def build_output(observations: list[dict[str, str | float]], *, cached: bool = False, as_of_date: str | None = None) -> dict[str, Any]:
    """Build the quarterly L4-007 result using approved 8Q and 20Q lookbacks."""
...
def run(*, raw_path: Path = RAW_PATH, output_path: Path = OUTPUT_PATH) -> dict[str, Any]:
    """Fetch GFDEGDQ188S, apply seven-day cache rules, and write output."""
    try:
