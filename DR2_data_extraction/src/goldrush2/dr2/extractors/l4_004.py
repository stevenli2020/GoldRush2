"""DR2 extractor for L4-004: 10-year breakeven inflation."""
...
VARIABLE_ID = "L4-004"
SERIES_ID = "T10YIE"
SOURCE_NAME = "FRED T10YIE - 10-Year Breakeven Inflation Rate"
...
def _valid(current: dict[str, str | float], comparison: dict[str, str | float], *, cached: bool) -> dict[str, Any]:
    ...
    if change < 0:
        signal, summary = 1, f"10Y breakeven inflation fell by {abs(change):.2f} percentage points, bullish for gold."
    elif change > 0:
        signal, summary = -1, f"10Y breakeven inflation rose by {change:.2f} percentage points, bearish for gold."
    else:
        signal, summary = 0, "10Y breakeven inflation was unchanged, neutral for gold."
    if cached:
        summary += " SOURCE UNAVAILABLE — cached data used."
    return {"signal": signal, "confidence": 1, "evidence": {"data": {"current_value": current_value, "current_date": current["date"], "comparison_value": comparison_value, "comparison_date": comparison["date"], "change_percentage_points": change}, "summary": summary}}


def build_output(observations: list[dict[str, str | float]], *, cached: bool = False, as_of_date: str | None = None) -> dict[str, Any]:
    """Build the four-horizon L4-004 result from valid FRED observations."""
...
def run(*, raw_path: Path = RAW_PATH, output_path: Path = OUTPUT_PATH) -> dict[str, Any]:
    """Fetch T10YIE, apply seven-day cache rules, and write current output."""
    try:
