...
CHECKMYSWAP_URL = "http://188.166.178.188/temp/datafeed/"
...
class OISCollector(BaseCollector):
    ...
    @staticmethod
    def _api_key() -> str:
        server_date = datetime.now(SERVER_TIMEZONE).strftime("%Y%m%d")
        return hashlib.sha256(f"OIS{server_date}".encode("ascii")).hexdigest()[:8]
    def _save_updated(self, existing: list[dict[str, Any]], incoming: list[dict[str, Any]], action: str) -> list[dict[str, Any]]:
        if self.force:
            ...
            self._log("forced OIS refresh merges with existing historical cache", 1)
        return super()._save_updated(existing, incoming, action)
    def _request(self, from_date: str) -> list[dict[str, Any]]:
        query = urlencode({"from": from_date.replace("-", ""), "api_key": self._api_key()}); self._log(f"request started: {CHECKMYSWAP_URL}?from={from_date.replace('-', '')}&api_key=********", 1)
        try:
