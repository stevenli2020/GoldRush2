...
from goldrush2.dr2.extractors import l4_003
...
def test_all_horizons_signal_directions(horizon, lookback, current, comparison, signal):
    output = l4_003.build_output(observations(comparison_index=756 - lookback, current=current, comparison=comparison), as_of_date="2026-09-01")
...
def test_insufficient_history_and_schema():
    output = l4_003.build_output(observations(4), as_of_date="2026-09-01")
    assert output["variable_id"] == "L4-003"
    assert set(output["horizons"]) == set(l4_003.HORIZON_LOOKBACKS)
...
def test_fresh_cache_fallback(monkeypatch, tmp_path):
    raw = tmp_path / "T5YIE.json"
    raw.write_text(json.dumps({"observations": [{"date": "2026-01-01", "value": "2"}]}) + "\n")
    monkeypatch.setattr(l4_003, "fetch_series", lambda *args, **kwargs: (_ for _ in ()).throw(FredError("offline")))
    output = l4_003.run(raw_path=raw, output_path=tmp_path / "out.json")
