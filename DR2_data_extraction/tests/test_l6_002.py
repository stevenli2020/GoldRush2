...
from goldrush2.dr2.extractors.l6_002 import run, _score
...
def test_snapshot(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr('goldrush2.dr2.collectors.ofac.requests.post', lambda *a, **k: (_ for _ in ()).throw(RuntimeError('offline')))
    monkeypatch.setattr('goldrush2.dr2.collectors.ofac.requests.head', lambda *a, **k: (_ for _ in ()).throw(RuntimeError('offline')))
