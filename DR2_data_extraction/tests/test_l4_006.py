...
from goldrush2.dr2.extractors import l4_006


def observations(count=20, comparison_index=None, current=-6.0, comparison=-5.5):
    rows = [{"date": f"{2020 + i // 4:04d}-{1 + (i % 4) * 3:02d}-01", "value": -5.0} for i in range(count)]
...
def test_quarterly_signal_directions_are_approved(horizon, lookback, current, comparison, signal):
    result = l4_006.build_output(observations(45, comparison_index=45-lookback, current=current, comparison=comparison))
...
def test_schema_and_quarterly_dates():
    ...
    assert result["horizons"]["1-3y"]["status"] == "VALID"


def test_missing_values_ignored():
    assert parse_observations({"observations": [{"date": "2026-01-01", "value": "."}, {"date": "2026-04-01", "value": "-6.2"}]}) == [{"date": "2026-04-01", "value": -6.2}]


def test_insufficient_history():
    result = l4_006.build_output(observations(7))
    assert result["horizons"]["1-3y"]["confidence"] == 0
    assert result["horizons"]["1-3y"]["status"] == "DEGRADED"


def test_empty_series_degrades_applicable_horizons():
    result = l4_006.build_output([])
...
def test_fresh_and_stale_cache(monkeypatch, tmp_path):
    raw = tmp_path / "FYFSGDA188S.json"
    raw.write_text(json.dumps({"observations": [{"date": "2026-01-01", "value": "-6"}]}) + "\n")
    monkeypatch.setattr(l4_006, "fetch_series", lambda *args, **kwargs: (_ for _ in ()).throw(FredError("offline")))
    fresh = l4_006.run(raw_path=raw, output_path=tmp_path / "fresh.json")
    assert "SOURCE UNAVAILABLE" in fresh["horizons"]["1-5d"]["evidence"]["summary"]
    old = time.time() - 8 * 86400
    os.utime(raw, (old, old))
    stale = l4_006.run(raw_path=raw, output_path=tmp_path / "stale.json")
