...
def test_discover_extractors_finds_all_current_modules():
    ...
    assert discovered["L0-009"] == "goldrush2.dr2.extractors.l0_009"
    assert discovered["L0-006"] == "goldrush2.dr2.extractors.l0_006"
    assert discovered["L7-003"] == "goldrush2.dr2.extractors.l7_003"


def test_extract_check_lists_discovered_mapping(monkeypatch, capsys):
    monkeypatch.setattr(cli, "discover_extractors", lambda: {"L0-006": "goldrush2.dr2.extractors.l0_006"})

    assert cli.main(["extract", "--check"]) == 0

    output = capsys.readouterr().out
    assert "L0-006: goldrush2.dr2.extractors.l0_006 [OK]" in output
...
def test_extract_rejects_missing_window_config(monkeypatch, tmp_path, capsys):
    def run(*, output_path: Path):
        return {"data_frequency": "Monthly", "observation_date": "2026-01-01"}
    monkeypatch.setattr(cli, "DR2_CURRENT_DIR", tmp_path)
    monkeypatch.setattr(cli, "discover_extractors", lambda: {"L0-006": "fake.extractor"})
    monkeypatch.setattr(cli.importlib, "import_module", lambda name: SimpleNamespace(run=run))
    assert cli.main(["extract", "L0-006"]) == 1
