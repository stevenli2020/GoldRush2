...
from goldrush2.extractors import l4_001
...
def test_all_monthly_horizon_directions(horizon, lookback, current, comparison, signal):
    output = l4_001.build_output(monthly_observations(comparison_index=756 - lookback, current=current, comparison=comparison), as_of_date="2026-09-01")
...
def test_monthly_dates_and_index_schema():
    output = l4_001.build_output(monthly_observations(6), as_of_date="2026-09-01")
    ...
    assert "CPI index" in output["horizons"]["1-5d"]["evidence"]["summary"]
...
def test_insufficient_history():
    output = l4_001.build_output(monthly_observations(4))
