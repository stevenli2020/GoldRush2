...
import pytest
import yaml
from goldrush2.analytics.aggregator import aggregate_horizon, load_weights, run_analytics
from goldrush2.analytics.models import VariableResult, HorizonSignal, Evidence, EvidenceData
...
def test_yaml_syntax_and_coverage():
    config = load_weights(Path("config/weights_v1.yaml"))
    assert "horizons" in config
    assert "L5-002" not in Path("config/weights_v1.yaml").read_text(encoding="utf-8")
