...
| DR3-10 | Plan 1 Step 2: structured input diagnostics | **Implemented; ready for review** | Per-variable contributions/status/reasons, usable-weight coverage and warnings. 22 focused WSL tests passed; Step 1 scoring preserved. |
| DR3-11 | Plan 1 Step 3: regression verification | **Implemented; ready for review** | 25 focused WSL tests passed. Added cancellation, missing-horizon/malformed-file checks, byte-preservation through real CLI dispatch, and isolated output tests. |
| DR3-12 | Plan 1 Step 4: comparison, coverage status and delta record | **Implemented; ready for review** | 27 focused WSL tests passed. Added Q's 70% coverage validity floor and fractional-confidence soft decay without renormalization; regenerated comparison and `SCORE_DELTA_REPORT.md` records 16 degraded results of 60. |
