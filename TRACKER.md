...
| DR3-13 | Plan 2 Task 1: source and frequency contract inventory | **Proposed for review** | Define exact source series/workbook, units, cadence, release lag, date basis, and variable-specific freshness limits for the priority inputs. No extractor changes yet. |
| DR3-14 | Plan 2 Task 2: approve variable and horizon evidence rules | **Proposed for review** | For each priority variable/horizon, approve evidence window, minimum observations, applicability, direction rationale, and confidence policy. Owner approval required before implementation. |
| DR3-15 | Plan 2 Task 3: implement frequency-aware extraction corrections | **Proposed for review** | Implement only approved rules, with fixtures for daily/weekly/monthly calendars, missing periods, release lag, units, and stale-source verification. |
| DR3-16 | Plan 2 Task 4: refresh, rerun, and review corrected outlook | **Proposed for review** | Refresh affected current JSON, rerun unchanged strategies, produce contribution-level deltas, and decide whether remaining variables need another correction tranche. |
