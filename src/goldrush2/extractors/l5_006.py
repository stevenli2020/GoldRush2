"""DR2 extractor for L5-006: monthly official-sector net reductions proxy."""
...
from goldrush2.extractors._wgc_common import HORIZON_LOOKBACKS, build_output as _build_output, degraded, parse_official_changes_workbook

VARIABLE_ID = "L5-006"
SOURCE_NAME = "WGC/IMF IFS Official-Sector Gold Net Reductions (proxy, tonnes)"
...
def parse_reductions_workbook(path: Path) -> list[dict[str, Any]]:
    """Aggregate negative canonical changes into positive monthly reductions."""
    totals: defaultdict[str, float] = defaultdict(float)
    for record in parse_official_changes_workbook(path):
        value = float(record["value"])
        if value < 0 and abs(value) >= DE_MINIMIS_TONNES:
            totals[str(record["date"])] += abs(value)
    if not totals:
        raise ValueError("WGC Monthly sheet contained no negative canonical changes")
...
def build_output(observations: list[dict[str, Any]], *, cached: bool = False, as_of_date: str | None = None) -> dict[str, Any]:
    """Build the four-horizon L5-006 output."""
    return _build_output(VARIABLE_ID, SOURCE_NAME, SOURCE_URL, observations, cached=cached, as_of_date=as_of_date, value_label="Official-sector net reductions", rising_signal=-1, falling_signal=1)
...
def run(*, output_path: Path = OUTPUT_PATH, raw_dir: Path = RAW_DIR) -> dict[str, Any]:
    """Collect, parse, and write the current L5-006 output."""
    ...
    if workbook is None:
        ...
        try:
            output = build_output(parse_reductions_workbook(workbook), cached=wgc.LAST_FETCH_USED_CACHE)
