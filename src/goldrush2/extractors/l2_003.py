"""DR2 extractor for L2-003: USD/CNY exchange rate."""
...
VARIABLE_ID = "L2-003"
SERIES_ID = "DEXCHUS"
SOURCE_NAME = "FRED DEXCHUS - Chinese Yuan to US Dollar Exchange Rate"
...
def _valid(current: dict[str, str | float], comparison: dict[str, str | float], *, cached: bool) -> dict[str, Any]:
    ...
    if change < 0:
        signal, summary = 1, f"USD/CNY fell by {abs(change):.4f} ({abs(change_pct or 0):.2f}%), bullish for gold."
    elif change > 0:
        signal, summary = -1, f"USD/CNY rose by {change:.4f} ({change_pct or 0:.2f}%), bearish for gold."
    else:
        signal, summary = 0, "USD/CNY was unchanged, neutral for gold."
    if cached:
...
def build_output(observations: list[dict[str, str | float]], *, cached: bool = False, as_of_date: str | None = None) -> dict[str, Any]:
    """Build the four-horizon L2-003 result from valid FRED observations."""
...
def run(*, raw_path: Path = RAW_PATH, output_path: Path = OUTPUT_PATH) -> dict[str, Any]:
    """Fetch DEXCHUS, apply seven-day cache rules, and write current output."""
    try:
