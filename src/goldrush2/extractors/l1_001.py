"""DR2 extractor for L1-001: 10Y TIPS Real Yield."""
...
SERIES_ID = "DFII10"
SOURCE_NAME = "FRED - 10-Year TIPS Real Yield (DFII10)"
...
OUTPUT_PATH = PROJECT_ROOT / "data" / "current" / "L1-001.json"
...
def _degraded_horizon(summary: str, data: dict[str, Any] | None = None) -> dict[str, Any]:
    return {
        "signal": 0,
        "confidence": 0,
        "evidence": {
            "data": data if data is not None else _empty_evidence_data(),
            "summary": summary,
        },
    }
...
def _valid_horizon(
    ...
    *,
    ...
    if change < 0:
        signal = 1
        summary = (
            f"The 10Y real yield fell by {abs(change):.2f} percentage points, "
            "bullish for gold."
        )
    elif change > 0:
        signal = -1
        summary = (
            f"The 10Y real yield rose by {change:.2f} percentage points, "
            "bearish for gold."
        )
    else:
        signal = 0
        summary = "The 10Y real yield was unchanged, neutral for gold."
...
def build_output(
    ...
    *,
    ...
    """Build the complete L1-001 JSON result from valid observations."""
    ...
    for horizon, lookback in HORIZON_LOOKBACKS.items():
        if current is None or len(ordered) < lookback:
            ...
            horizons[horizon] = _degraded_horizon(
                "MISSING DATA — "
                f"{lookback} valid observations are required; {len(ordered)} are available.",
                data,
            )
            continue
...
def build_degraded_output(summary: str, *, as_of_date: str | None = None) -> dict[str, Any]:
    """Build a four-horizon zero-confidence result for a collection failure."""
    return {
        "variable_id": "L1-001",
        "as_of_date": as_of_date or date.today().isoformat(),
        "source_name": SOURCE_NAME,
...
def run(*, raw_path: Path = RAW_PATH, output_path: Path = OUTPUT_PATH) -> dict[str, Any]:
    """Collect DFII10, apply cache rules, and write the current L1-001 output."""
    try:
        ...
        if raw_path.exists() and _cache_is_fresh(raw_path):
            ...
            output = build_degraded_output(
                f"STALE DATA — FRED unavailable and cached data is 7 days old or older: {exc}"
            )
        else:
