"""DR2 extractor for L1-002: 5Y TIPS Real Yield."""
...
SERIES_ID = "DFII5"
SOURCE_NAME = "FRED - 5-Year TIPS Real Yield (DFII5)"
SOURCE_URL = "https://fred.stlouisfed.org/series/DFII5"
...
OUTPUT_PATH = PROJECT_ROOT / "data" / "current" / "L1-002.json"
...
def _valid_horizon(
    ...
    *,
    ...
    if change < 0:
        signal = 1
        summary = (
            f"The 5Y real yield fell by {abs(change):.2f} percentage points, "
            "bullish for gold."
        )
    elif change > 0:
        signal = -1
        summary = (
            f"The 5Y real yield rose by {change:.2f} percentage points, "
            "bearish for gold."
        )
    else:
        signal = 0
        summary = "The 5Y real yield was unchanged, neutral for gold."
...
def build_output(
    ...
    *,
    ...
    """Build the complete L1-002 JSON result from valid observations."""
...
def run(*, raw_path: Path = RAW_PATH, output_path: Path = OUTPUT_PATH) -> dict[str, Any]:
    """Collect DFII5, apply cache rules, and write the current L1-002 output."""
    try:
